...
BOOST_DATA_TEST_CASE(allAlps, bdata::make(editeddata["Alps"]), sample)
{
	BOOST_TEST(m.checkRange(sample, "Alps"), "FALSE NEGATIVE: " + sample + " should be in the Alps");
}

BOOST_DATA_TEST_CASE(allCarpathians, bdata::make(editeddata["Carpathians"]), sample)
{
	BOOST_TEST(m.checkRange(sample, "Carpathians"), "FALSE NEGATIVE: " + sample + " should be in the Carpathians");
}

BOOST_DATA_TEST_CASE(allIcelandicHighlands, bdata::make(editeddata["Icelandic Highlands"]), sample)
{
	BOOST_TEST(m.checkRange(sample, "Icelandic Highlands"), "FALSE NEGATIVE: " + sample + " should be in the Icelandic Highlands");
}

BOOST_DATA_TEST_CASE(RandomPyrenees, bdata::make(editeddata["Pyrenees"]), sample)
{
...
BOOST_DATA_TEST_CASE(RandomWrongAnswers, bdata::random(0, 11999) ^ bdata::xrange(50), sample, idx)
{
	...
	BOOST_TEST(!m.checkRange(editeddata[mRange][sample % editeddata[mRange].size()], wRange), "FALSE POSITIVE: " + editeddata[mRange][sample % editeddata[mRange].size()] + " is not in the " + wRange);
}
